import {
  ...
  // Custom tick component for X-axis labels with footnotes
  const CustomXAxisTick = (props: any) => {
    const { x, y, payload } = props;
    
    // Parse the value to separate label and footnote
    // Format is "Label|||footnoteNumber" if there's a footnote
    ...
    return (
      <g transform={`translate(${x},${y})`}>
        ...
        >
          {label}
          {footnoteNumber && (
            <tspan baselineShift="super" fontSize={8}>{footnoteNumber}</tspan>
          )}
        </text>
      </g>
    );
